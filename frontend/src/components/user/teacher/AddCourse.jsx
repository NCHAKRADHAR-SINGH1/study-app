...
const AddCourse = () => {
   ...
   const handleSubmit = async (e) => {
      ...
      // Validation
      if (!addCourse.C_title || !addCourse.C_educator || !addCourse.C_categories || !addCourse.C_description) {
      ...
      for (let i = 0; i < addCourse.sections.length; i++) {
         if (!addCourse.sections[i].S_title || !addCourse.sections[i].S_description || !addCourse.sections[i].S_content) {
            alert(`Section ${i + 1}: Please fill all fields and upload a video`);
            return;
         }
         // Check file type
         const file = addCourse.sections[i].S_content;
         if (file.type !== 'video/mp4') {
            alert(`Section ${i + 1}: Only MP4 videos are allowed. You selected: ${file.type || file.name}`);
            return;
         }
      }
      ...
      try {
         ...
         alert(error.response?.data?.message || 'Error creating course: ' + error.message);
      }
   };
